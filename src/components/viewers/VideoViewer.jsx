...
const VideoWithAdvancedFeatures = ({ url, mimeType }) => {
  // Extract values from user context.
   ...
   // New state for inTime (first time user starts watching)
   ...
   const handlePlay = () => {
     ...
     // Set inTime only if it's not already set.
     if (!inTime) {
       setInTime(new Date().toISOString());
     }
     setAnalytics((prev) => {
   ...
   useEffect(() => {
     const interval = setInterval(() => {
       ...
       updatedAnalytics = {
         ...updatedAnalytics,
         // Keep the original inTime if set; otherwise, it can be null.
         inTime: inTime,
     ...
     return () => clearInterval(interval);
   }, [ip, location, userId, region, os, device, browser, url, inTime, playedSeconds, videoEl]);
